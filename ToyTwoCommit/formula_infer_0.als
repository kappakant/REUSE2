...
one sig Abort extends BaseName {} {
	paramIdxs = P0
	paramTypes = P0->RMs
}
one sig Abortres2 extends Act {} {
	params = (P0->res2)
}
one sig Abortres1 extends Act {} {
	params = (P0->res1)
}

one sig Commit extends BaseName {} {
	paramIdxs = P0
	paramTypes = P0->RMs
}
one sig Commitres2 extends Act {} {
	params = (P0->res2)
}
one sig Commitres1 extends Act {} {
	params = (P0->res1)
}

one sig Prepare extends BaseName {} {
	paramIdxs = P0
	paramTypes = P0->RMs
}
one sig Prepareres2 extends Act {} {
	params = (P0->res2)
}
one sig Prepareres1 extends Act {} {
	params = (P0->res1)
}
one sig Prepareres3 extends Act {} {
	params = (P0->res3)
}
...
fact {
	...
	Abort in FlSymAction.baseName // the final base name in the neg trace must appear in the sep formula
}
...
fact PartialInstance {
	lastIdx = (EmptyTrace->T0) + (PT1->T5) + (PT2->T1) + (NT1->T5)

	path = (PT1 -> (T0->Prepareres1 + T1->Prepareres2 + T2->Prepareres3 + T3->Commitres1 + T4->Commitres2 + T5->Commitres1)) +
		(PT2 -> (T0->Prepareres1 + T1->Abortres1)) +
		(NT1 -> (T0->Prepareres1 + T1->Prepareres2 + T2->Prepareres3 + T3->Commitres1 + T4->Commitres2 + T5->Abortres2))

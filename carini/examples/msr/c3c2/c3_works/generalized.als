...
sig Fluent extends Formula {
    ...
    ParamIdx.(flActions.actToFlParamsMap) = vars.Var
    ...
    //all x1,x2,y1,y2 : ParamIdx |
        //(x1->y1 in flActions.actToFlParamsMap and x2->y2 in flActions.actToFlParamsMap) implies (x1->y1 = x2->y2 or (not x1=x2 and not y1=y2))
...
pred isInitAct[a : Act, e : Env, f : Fluent] {
    ...
    (some s : f.flActions |
        s.value = True and a.baseName = s.baseName and (~(f.vars)).(~(s.actToFlParamsMap)).(a.params) in e.maps)
    or
    // mutex term fluents
    (some s : f.flActions |
        s.mutexFl = True and s.value = False and a.baseName = s.baseName and (~(f.vars)).(~(s.actToFlParamsMap)).(a.params) not in e.maps)
}
...
pred isTermAct[a : Act, e : Env, f : Fluent] {
    ...
    (some s : f.flActions |
        s.value = False and a.baseName = s.baseName and (~(f.vars)).(~(s.actToFlParamsMap)).(a.params) in e.maps)
    or
    // mutex init fluents
    (some s : f.flActions |
        s.mutexFl = True and s.value = True and a.baseName = s.baseName and (~(f.vars)).(~(s.actToFlParamsMap)).(a.params) not in e.maps)
}
...
fact {
	...
	//BecomeLeader in FlSymAction.baseName
...
fact PartialInstance {
	//lastIdx = (EmptyTrace->T0) + (PT37->T2) + (PT19->T2) + (PT50->T2) + (PT1->T0) + (PT47->T3) + (PT42->T2) + (PT51->T2) + (PT44->T1) + (PT30->T3) + (PT46->T2) + (PT36->T2) + (PT43->T2) + (NT1->T9)
	lastIdx = (EmptyTrace->T0) + (PT1->T0) + (NT1->T9)

	path = //(PT37 -> (T0->GetEntriesn1n1 + T1->GetEntriesn2n1 + T2->GetEntriesn3n1)) +

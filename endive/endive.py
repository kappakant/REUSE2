...
JAVA_EXE="java"
GEN_TLA_DIR="gen_tla"
MIN_NUM_INIT_STATES = 100 # min num init states needed to declare inductive
...
def runtlc_bfs(spec,config=None,tlc_workers=6,cwd=None,java="java",tlc_flags="",timeout=None):
...
def runtlc_sim(spec,config=None,tlc_workers=6,cwd=None,java="java",tlc_flags="",timeout=None):
    num_sims = 1000
    # Make a best effort to attempt to avoid collisions between different
    # instances of TLC running on the same machine.
    dirpath = tempfile.mkdtemp()
    metadir_path = f"states/states_{random.randint(0,1000000000)}"
    simulate_cmd = f"-simulate num={num_sims} -depth 15"
    cmd = java + (f' -Djava.io.tmpdir="{dirpath}" -cp tla2tools-checkall.jar tlc2.TLC {tlc_flags} {simulate_cmd} -maxSetSize {TLC_MAX_SET_SIZE} -metadir {metadir_path} -noGenerateSpecTE -checkAllInvariants -deadlock -continue -workers {tlc_workers}')
    if config:
        cmd += " -config " + config
    cmd += " " + spec
    if timeout is not None:
...
def num_init_states_sim_mode(lines):
    for l in greplines(" initial states...", lines):
        res = re.match(".*Computed ([0-9]*) initial states...",l)
        num = res.group(1)
        return int(num)
    return None

def detect_inv_equals_false_error(lines):
    for l in greplines("is equal to FALSE", lines):
        res = re.match(".*The invariant of (Inv.*) is equal to FALSE",l)
        invname = res.group(1)
        return invname
    return None
...
def runtlc_check_violated_invariants(runtlc, spec, config=None, tlc_workers=6, cwd=None,java="java", timeout=None):
    #
    ...
    num_false_invs = 0
    false_inv = ""
    while false_inv is not None:
        lines = runtlc(spec,config=config,tlc_workers=tlc_workers,cwd=cwd,java=java,timeout=timeout)
        false_inv = detect_inv_equals_false_error(lines)
        if false_inv is not None:
            num_false_invs += 1
            invs_violated.add(false_inv)
            replace_cmd = f"sed -i '' 's|^{false_inv}.*$|& \\\\/ TRUE|g' {spec}.tla"
            subprocess.Popen(replace_cmd, shell=True, cwd=cwd)
    logging.info(f"Removed {num_false_invs} false invs by making them equal to TRUE")
...
class InductiveInvGen():
    ...
    def __init__(self, specdir, specname, safety, constants, state_constraint, quant_inv, model_consts, preds,
        ...
        self.MAX_NUM_CTIS_PER_ROUND = 500 #10000
    ...
    def create_check_invariants_files(self, inv_names, invs, tlc_workers=6):
        invcheck_tla = "---- MODULE %s_InvCheck_%d ----\n" % (self.specname,self.seed)
        invcheck_tla += "EXTENDS %s\n\n" % self.specname
        invcheck_tla += self.model_consts + "\n"

        for i,inv in enumerate(invs):    
            if ("Inv%d" % i) in inv_names:
    ...
    def check_invariants(self, invs, tlc_workers=6, timeout=None, only_check=None):
        """ Check which of the given invariants are valid. """
        ...
        for i,inv in enumerate(invs):
            inv_name = "Inv%d" % i
            use_inv = only_check is None or inv_name in only_check
            if use_inv:
                all_inv_names.add(inv_name)

        # Check invariants.
        self.create_check_invariants_files(all_inv_names, invs, tlc_workers)
        logging.info("Checking %d candidate invariants in spec file '%s'" % (len(all_inv_names), invcheck_spec_name))
        workdir = None if self.specdir == "" else self.specdir
        ta = time.time()
        violated_invs = runtlc_check_violated_invariants(runtlc_bfs, invcheck_spec_name, config=invcheck_cfg_filename, tlc_workers=self.tlc_workers, cwd=workdir,java=self.java_exe, timeout=timeout)
        sat_invs = all_inv_names - violated_invs
        logging.info(f"Found {len(sat_invs)} / {len(all_inv_names)} candidate invariants satisfied in {round(time.time()-ta,2)}s.")

        """
        # Check invariants. (BFS mode)
        self.create_check_invariants_files(all_inv_names, invs, tlc_workers)
        logging.info("Checking %d candidate invariants in spec file '%s'" % (len(invs), invcheck_spec_name))
        workdir = None if self.specdir == "" else self.specdir
        ta = time.time()
        violated_invs = runtlc_check_violated_invariants(runtlc_bfs, invcheck_spec_name, config=invcheck_cfg_filename, tlc_workers=self.tlc_workers, cwd=workdir,java=self.java_exe, timeout="3m")
        all_inv_names = (all_inv_names - violated_invs)
        logging.info(f"Found {len(all_inv_names)} / {len(invs)} candidate invariants satisfied in {round(time.time()-ta,2)}s.")

        # Check invariants. (sim mode)
        self.create_check_invariants_files(all_inv_names, invs, tlc_workers)
        logging.info("Checking %d candidate invariants in spec file '%s'" % (len(all_inv_names), invcheck_spec_name))
        workdir = None if self.specdir == "" else self.specdir
        ta = time.time()
        violated_invs = runtlc_check_violated_invariants(runtlc_sim, invcheck_spec_name, config=invcheck_cfg_filename, tlc_workers=self.tlc_workers, cwd=workdir,java=self.java_exe, timeout="15m")
        sat_invs = (all_inv_names - violated_invs)
    ...
    def parse_ctis(self, lines):
        ...
        while curr_line < len(lines) and not re.search('Error: The behavior up to this point is', lines[curr_line]):
            curr_line += 1
    ...
    def generate_ctis_tlc_run_async(self, num_traces_per_worker):
        ...
        #args = (dirpath, ctiseed, tag, num_ctigen_tlc_workers, indcheckcfgfilename, indchecktlafilename)
        #cmd = self.java_exe + ' -Xss16M -Djava.io.tmpdir="%s" -cp tla2tools-checkall.jar tlc2.TLC -seed %d -noGenerateSpecTE -metadir states/indcheckrandom_%d -continue -deadlock -workers %d -config %s %s' % args
        args = (dirpath, TLC_MAX_SET_SIZE, simulate_flag, self.simulate_depth, ctiseed, tag, num_ctigen_tlc_workers, indcheckcfgfilename, indchecktlafilename)
    ...
    def generate_ctis_tlc_run_await(self, subproc):
        ...
        # Detect the case where no init state are generated
        #if "Error: There is no state satisfying the initial state predicate." in tlc_out:
            #return (0,False)

        parsed_ctis = self.parse_ctis(lines)     
        num_init_states = num_init_states_sim_mode(lines)
        return (parsed_ctis, num_init_states)
    ...
    def generate_ctis(self):
        ...
        while len(all_ctis) == 0 and num_init_states < MIN_NUM_INIT_STATES:
            ...
            for cti_subproc in cti_subprocs:
                ...
                if self.use_apalache_ctigen:
                    ...
                    parsed_ctis,init_states_subproc = self.generate_ctis_tlc_run_await(cti_subproc) 
                    num_init_states += init_states_subproc 
                all_ctis = all_ctis.union(parsed_ctis)

            num_gen_iters += 1
            dur_so_far = int(time.time() - self.ctigen_start)
            print(f"Total number of init states generated: {num_init_states}")
            print(f"Time spent for CTI generation so far: {dur_so_far} sec")
            print(f"CTI generation has been run {num_gen_iters} time(s) so far this round")

            if len(all_ctis) == 0 and num_init_states < MIN_NUM_INIT_STATES:
                print(f"The number of init states is below the minimum ({MIN_NUM_INIT_STATES}) so we will rerun CTI generation")
                print("")
    ...
    def eliminate_ctis(self, orig_k_ctis, num_invs, roundi, preds_alt=[], quant_inv_alt=None, tlc_workers=6, specdir=None):
        ...
        while iteration <= self.num_iters:
            ...
            # makes the previous several lines unneeded
            min_conjs = iteration+1
            max_conjs = iteration+1
            ...
            if self.use_cpp_invgen:
                ...
                sat_invs = self.check_invariants(invs, tlc_workers=tlc_workers, timeout="3m")
            ...
            while curr_ind < len(sat_invs):
                ...
                for ci,subproc in enumerate(tlc_procs):
                    ...
                    text = fcti.read()
                    if not text:
                        raise Exception("trying to parse empty CTI file: " + cti_states_file)
            ...
            # Perform another round of invariant checking for only the inv candidates that eliminate at least one CTI.
            logging.info("")
            logging.info(f"Performing extra invariant checks (in iteration {iteration})")

            # Double check
            elim_ctis = set([inv for inv in cti_states_eliminated_by_invs if len(cti_states_eliminated_by_invs[inv]) > 0])
            logging.info(f"Double checking {len(elim_ctis)} candidate invariants (that eliminate at least one CTI) for 15m")
            sat_invs = self.check_invariants(invs, tlc_workers=tlc_workers, timeout="15m", only_check=elim_ctis)

            # Triple check
            if len(sat_invs) > 0:
                elim_ctis = sat_invs
                timeout = 10 + 5*iteration + int(len(elim_ctis)/5)
                timeout_str = f"{timeout}m"
                logging.info(f"Triple checking {len(elim_ctis)} candidate invariants (that eliminate at least one CTI) for {timeout_str}")
                sat_invs = self.check_invariants(invs, tlc_workers=tlc_workers, timeout=timeout_str, only_check=elim_ctis)

            sat_invs = list(sorted(sat_invs))
            self.all_sat_invs = self.all_sat_invs.union(set(map(get_invexp, list(sat_invs))))
            unsat_invs = set(cti_states_eliminated_by_invs.keys()) - set(sat_invs)
            for unsat_inv in unsat_invs:
                del cti_states_eliminated_by_invs[unsat_inv]

            """
            elim_ctis = set([inv for inv in cti_states_eliminated_by_invs if len(cti_states_eliminated_by_invs[inv]) > 0])
            timeout = 10 + 5*iteration + int(len(elim_ctis)/100)
            timeout_str = f"{timeout}m"
            logging.info(f"Double checking {len(elim_ctis)} candidate invariants (that eliminate at least one CTI) for {timeout_str}")
            sat_invs = self.check_invariants(invs, tlc_workers=tlc_workers, timeout=timeout_str, only_check=elim_ctis)
            sat_invs = list(sorted(sat_invs))
            self.all_sat_invs = self.all_sat_invs.union(set(map(get_invexp, list(sat_invs))))
            unsat_invs = set(cti_states_eliminated_by_invs.keys()) - set(sat_invs)
        ...
        if num_ctis_remaining > 0:
            ctis_remaining = set(cti_table.values()) - eliminated_ctis
            example_cti = list(ctis_remaining)[0]
            print(f"Failed to eliminate all CTIs, here is an example CTI:\n{example_cti}")
            return None
        return self.strengthening_conjuncts
